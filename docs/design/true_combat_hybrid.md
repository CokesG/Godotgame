...
- Class selection now happens before `Deal In`, and `RunManager` deals a class-specific 10-card starter deck: Gambler-Knight stays balanced, Hex Sharpshooter opens with read/trap/mobility tools, and Blood Wager opens with ritual/overclock/guard pressure.
- FPS card-power slots now use compact framed icon labels with class-accent borders. Ability rings blend the ability family color with the active class accent, so the same card power feels different when cast by a different fighter.
...
- The prep table also has an armory readout: active arena reward mods, Card XP, wounds, and the predicted recommended kit with chip cost are visible before entering the shooter. Reward mods have rarity and objective-bias tags, and they are recorded in run reward history.
